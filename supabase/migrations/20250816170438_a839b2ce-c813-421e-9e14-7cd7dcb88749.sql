-- Supprimer toutes les versions de la fonction update_member_info pour éviter les conflits
DROP FUNCTION IF EXISTS update_member_info(member_id INTEGER, update_data JSONB);
DROP FUNCTION IF EXISTS update_member_info(member_id BIGINT, update_data JSONB);

-- Créer une seule version avec le bon type (BIGINT car c'est le type de la colonne id dans members)
...
BEGIN
  ...
  -- Mettre à jour les informations du membre avec les vrais noms de colonnes
  UPDATE members 
  SET 

-- Nettoyer la table contacts : supprimer la colonne "statut" en double
-- Copier les données de "statut" vers "status" si nécessaire
UPDATE public.contacts 
SET status = COALESCE(statut, status) 
WHERE statut IS NOT NULL AND statut != status;

-- Supprimer la colonne "statut" en double
ALTER TABLE public.contacts DROP COLUMN IF EXISTS statut;

-- Recréer la fonction get_contact_messages sans la colonne statut
...
BEGIN
  ...
  ORDER BY 
    ...
    CASE WHEN contacts.status = 'géré' THEN 1 ELSE 0 END ASC,

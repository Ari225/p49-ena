...
const ActualitesConcours = () => {
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'bientot': return 'bg-yellow-100 text-yellow-800';
  ...
  const getStatusText = (status: string) => {
    switch (status) {
      ...
      case 'bientot': return 'Bientôt ouvert';
      case 'ferme': return 'Inscriptions fermées';
        ...
        <div className={`container mx-auto py-16 ${isMobile ? 'px-[25px]' : 'px-[100px]'}`}>
          ...
          <div className="flex flex-wrap gap-4 mb-8 justify-center">
            ...
            <button
              ...
              className={`px-4 py-2 rounded-lg font-medium transition-colors ${
                filter === 'admission' ? 'bg-primary text-white' : 'bg-white text-gray-600 hover:bg-gray-100'
              }`}
            >
              Concours d'admission
            </button>
            <button
              onClick={() => setFilter('fonction-publique')}
              className={`px-4 py-2 rounded-lg font-medium transition-colors ${
                filter === 'fonction-publique' ? 'bg-primary text-white' : 'bg-white text-gray-600 hover:bg-gray-100'
              }`}
            >
              Fonction publique
            </button>
            <button
              onClick={() => setFilter('specialise')}
              className={`px-4 py-2 rounded-lg font-medium transition-colors ${
                filter === 'specialise' ? 'bg-primary text-white' : 'bg-white text-gray-600 hover:bg-gray-100'
              }`}
            >

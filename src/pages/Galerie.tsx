...
const Galerie = () => {
  ...
  const filteredPhotos = selectedCategory === 'Toutes' ? photos : photos.filter(photo => photo.category === selectedCategory);
  return <Layout>
        ...
        <section className="py-8 px-[100px] bg-accent/20">
          <div className="container mx-auto px-4">
            <div className="flex flex-wrap gap-2">
              {categories.map(category => <Button key={category} variant={selectedCategory === category ? "default" : "outline"} onClick={() => setSelectedCategory(category)} className={selectedCategory === category ? "bg-primary" : ""}>
                  {category}
                </Button>)}
            </div>
          </div>
        ...
        <section className="px-[100px] py-[100px]">
          <div className="container mx-auto px-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
              {filteredPhotos.map(photo => <Card key={photo.id} className="overflow-hidden hover:shadow-lg transition-shadow duration-300">
                  <Dialog>
                    <DialogTrigger asChild>
                      <div className="aspect-video overflow-hidden cursor-pointer">
                        <img src={photo.url} alt={photo.title} className="w-full h-full object-cover hover:scale-105 transition-transform duration-300" />
                      </div>
                    </DialogTrigger>
                    <DialogContent className="max-w-4xl max-h-[90vh] p-0">
                      <div className="relative">
                        <img src={photo.url.replace('w=400&h=300', 'w=1200&h=800')} alt={photo.title} className="w-full h-auto object-contain max-h-[80vh]" />
            ...
            {filteredPhotos.length === 0 && <div className="text-center py-12">

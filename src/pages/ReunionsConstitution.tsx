...
const ReunionsConstitution = () => {
  ...
  const fetchMediaItems = async () => {
    try {
      ...
      const filtered = (data || []).filter(
        (item: MediaItem) => item.category === 'Réunions de constitution'
      );
  ...
  const MediaSection = () => (
    <section className={`py-16 bg-white ${isMobile ? 'px-[25px]' : isTablet ? 'px-[50px]' : 'px-8 lg:px-[100px]'}`}>
      <div className="container mx-auto px-0">
        ...
        {loadingMedia ? (
          <div className="text-center py-8 text-gray-500">Chargement des médias...</div>
        ) : mediaItems.length === 0 ? (

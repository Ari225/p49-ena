            ...
            <div className="max-w-6xl mx-auto">
              ...
              <div className="relative">
                ...
                {timeline.map((event, index) => <div key={index} className={`relative flex items-center mb-16 ${index % 2 === 0 ? 'justify-start' : 'justify-end'}`}>
                    ...
                    {/* Content Card */}
                    <Card className={`w-5/12 ${index % 2 === 0 ? 'mr-auto' : 'ml-auto'} shadow-xl hover:shadow-2xl transition-all duration-500 hover:scale-105 border-l-4 ${index % 2 === 0 ? 'border-l-primary' : 'border-l-secondary'}`}>
                      <CardContent className="p-8">
                        <div className="flex items-center mb-4">
                          <div className={`${index % 2 === 0 ? 'bg-primary text-white' : 'bg-secondary text-primary'} font-bold text-lg px-4 py-2 rounded-lg shadow-md`}>
                            {event.year}
                          </div>
                        </div>
                        <h3 className={`text-xl font-bold mb-4 ${index % 2 === 0 ? 'text-primary' : 'text-secondary'}`}>
                          {event.title}
                        </h3>

  ...
  const fetchActualite = async () => {
    try {
      ...
      const {
        data: newsData,
        error
      } = await supabase.from('news').select('*').eq('id', id).eq('is_visible', true).single();
      if (error) {
          ...
          <article className="bg-white rounded-2xl shadow-sm overflow-hidden">
            ...
            <div className="relative">
              ...
              <div className="absolute top-4 left-4">
                <span className="bg-primary text-white px-2 py-1 rounded text-xs font-medium inline-flex items-center">
            ...
            <div className={`${isMobile ? 'p-6' : isTablet ? 'p-8' : 'p-10'}`}>
              ...
              <div className="prose prose-lg max-w-none [&_a]:pointer-events-auto [&_a]:cursor-pointer [&_a]:relative [&_a]:z-10">
                {actualite.content ? <div className="text-gray-700 leading-relaxed whitespace-pre-wrap" style={{
                lineHeight: '1.8'
              ...
              {process.env.NODE_ENV === 'development' && <div className="mt-4 p-4 bg-gray-100 rounded text-xs">

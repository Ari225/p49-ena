  ...
  return (
    <header className="bg-white shadow-lg border-b border-gray-200 sticky top-0 z-50">
              ...
              <div className="flex items-center space-x-1">
                <button
                  ...
                  className={`px-2 py-1 text-sm font-medium rounded transition-colors ${
                    language === 'fr' ? 'bg-primary text-white' : 'text-gray-700 hover:bg-accent'
                  }`}
                >
                  FR
                </button>
                <button
                  ...
                  className={`px-2 py-1 text-sm font-medium rounded transition-colors ${
                    language === 'en' ? 'bg-primary text-white' : 'text-gray-700 hover:bg-accent'
                  }`}
                >
                  EN
                </button>
      ...
      <div className="lg:hidden">
        <div className="flex items-center justify-between px-[25px] py-3">
          ...
          <div className="flex items-center space-x-2 flex-1">
            <Link to="/" className="flex items-center space-x-2">
              ...
              <span className="font-bold text-primary text-sm">P49 ENA</span>
            </Link>
          </div>
        ...
        {/* Mobile Menu Overlay */}
        {isMobileMenuOpen && (
          <div className="absolute top-full left-0 right-0 bg-white shadow-lg border-t border-gray-200 max-h-[calc(100vh-60px)] overflow-y-auto z-40">
            <nav className="py-4">
              {menuItems.map((menu) => (
                <div key={menu.title}>
                  <button
                ...
                {user ? (
                  <div className="flex-1">
                    <Link
                      to="/dashboard"
                      onClick={closeMobileMenu}
                      className="block text-center px-4 py-2 text-sm font-medium text-primary border border-primary rounded hover:bg-primary hover:text-white transition-colors"
                    >
                      Dashboard
                    </Link>
                  </div>

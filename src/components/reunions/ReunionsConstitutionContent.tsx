...
const ReunionsConstitutionContent = () => {
  ...
  const ReunionCardMobile = ({ reunion }: { reunion: Reunion }) => (
      ...
      <CardContent>
        ...
        {reunion.objectifs && (
          <div className="mb-3">
            ...
            <ul className="text-xs text-gray-600 space-y-1">
              {reunion.objectifs.slice(0, 2).map((objectif: string, index: number) => (
                <li key={index} className="flex items-start">
                  <span className="text-primary mr-1">•</span>
                  {objectif}
                </li>
              ))}
              {reunion.objectifs.length > 2 && (
                <li className="text-gray-500">... et {reunion.objectifs.length - 2} autres</li>
              )}
            </ul>
          </div>
        )}
        ...
        {reunion.resultats && (
          <div>
            ...
            <ul className="text-xs text-gray-600 space-y-1">
              {reunion.resultats.slice(0, 2).map((resultat: string, index: number) => (
                <li key={index} className="flex items-start">
  ...
  const ReunionCardTablet = ({ reunion }: { reunion: Reunion }) => (
    <Card className="hover:shadow-lg transition-shadow">
      <CardHeader>
        <div className="flex items-center justify-between">
  ...
  const ReunionCardDesktop = ({ reunion }: { reunion: Reunion }) => (
    <Card className="hover:shadow-lg transition-shadow">
      <CardHeader>
        <div className="flex items-center justify-between">
          <CardTitle className="text-primary">{reunion.titre}</CardTitle>
            ...
            <div>
              ...
              <div className="space-y-4">
                {reunionsProgrammees.map((reunion) => (
                  <ReunionCardMobile key={reunion.id} reunion={reunion} />
                ))}
              </div>
            </div>
          )}
          ...
          {selectedTab === 'programmees' && (
            <div>
              ...
              <div className="grid grid-cols-1 gap-6">
                {reunionsProgrammees.map((reunion) => (
                  <ReunionCardTablet key={reunion.id} reunion={reunion} />
                ))}
              </div>
            </div>
          )}
        ...
        {selectedTab === 'programmees' && (
          <div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              {reunionsProgrammees.map((reunion) => (
                <ReunionCardDesktop key={reunion.id} reunion={reunion} />
              ))}
            </div>
          </div>
        )}

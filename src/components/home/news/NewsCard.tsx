...
const NewsCard: React.FC<NewsCardProps> = ({
  ...
  return <Link to={`/actualite/${item.id}`}>
      <Card className="overflow-hidden hover:shadow-xl transition-all duration-300 group cursor-pointer bg-white border border-gray-200 h-full">
        <div className="aspect-[16/10] overflow-hidden relative">
          <img src={item.image_url} alt={item.title} className={`w-full h-full object-cover transition-transform duration-500 ${variant === 'desktop' ? 'group-hover:scale-110' : 'group-hover:scale-105'}`} />
          ...
          {isCenter && variant === 'desktop' && <div className="absolute inset-0 bg-gradient-to-b from-transparent via-transparent to-black/20"></div>}
        </div>
        <CardContent className="p-6">
          ...
          <h3 className={`font-bold text-primary leading-tight line-clamp-2 mb-3 ${isCenter && variant === 'desktop' ? 'text-xl' : variant === 'mobile' ? 'text-xl' : 'text-lg'}`}>
            {item.title}
          </h3>
          <p className="text-gray-600 line-clamp-3 leading-relaxed mb-4 text-sm">
            {item.summary}
          </p>
          {showReadButton && <div className="flex items-center justify-between">
              <Button variant="ghost" size="sm" className="text-primary hover:text-white hover:bg-primary transition-all duration-300">
                <Eye className="h-4 w-4 mr-2" />
                Lire l'article
              </Button>
              {variant !== 'mobile' && <ArrowRight className="h-4 w-4 text-gray-400 group-hover:text-primary transition-colors duration-300" />}
            </div>}
          {variant === 'mobile'}
        </CardContent>
      </Card>

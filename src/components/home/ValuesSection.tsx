...
const ValuesSection = () => {
  ...
  return <section className={`bg-white ${isMobile ? 'py-[50px] px-[25px]' : 'py-12 md:py-16 lg:py-[100px] px-4 md:px-8 lg:px-[100px]'}`}>
      <div className="container mx-auto px-0">
        <h2 className={`${isMobile ? 'text-xl' : 'text-2xl md:text-3xl'} text-center text-primary mb-6 md:mb-8 font-bold`}>
          Valeurs de la P49
        </h2>
        ...
        {isMobile ? <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              {[values[0], values[1]].map((value, index) => <div key={index} className="text-center grid place-items-center">
            ...
            <div className="grid grid-cols-2 gap-4">
              {[values[2], values[3]].map((value, index) => <div key={index} className="text-center grid place-items-center">
                  <div className="w-10 h-10 rounded-full flex items-center justify-center mx-auto mb-2 bg-secondary/80">
                    <value.icon className="h-4 w-4 text-primary" />
                  </div>
                  <h3 className="text-primary mb-1 text-base font-semibold">
            ...
            {values.map((value, index) => <div key={index} className="text-center">
                ...
                <p style={{
            minHeight: '3rem'
          }} className="text-sm leading-relaxed text-gray-700 text-center font-normal px-[50px]">
                  {value.description}
                </p>
              </div>)}
          </div>}

  ...
  return <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="w-[95%] max-w-4xl max-h-[90vh] overflow-y-auto rounded-lg mx-auto p-0" hideCloseButton>
        <DialogHeader className="p-6 pb-0">
          <div className="flex items-center justify-between">
            <DialogTitle className="text-primary text-lg">
              {mediaItem.alt}
              {canNavigate && <span className="text-sm font-normal text-muted-foreground ml-2">
        ...
        <div className="p-6 pt-0">
          <div className="relative w-full rounded-lg overflow-hidden bg-black" ref={contentRef} onTouchStart={handleTouchStart} onTouchMove={handleTouchMove} onTouchEnd={handleTouchEnd}>
            {/* Navigation buttons for all devices */}
            {canNavigate && <>
                <button onClick={handlePrevious} disabled={isFirst} className="absolute left-4 top-1/2 -translate-y-1/2 z-10 p-2 bg-black/50 hover:bg-black/70 disabled:bg-black/20 disabled:cursor-not-allowed rounded-full transition-colors">
                  <ChevronLeft className="h-6 w-6 text-white" />
                </button>
                <button onClick={handleNext} disabled={isLast} className="absolute right-4 top-1/2 -translate-y-1/2 z-10 p-2 bg-black/50 hover:bg-black/70 disabled:bg-black/20 disabled:cursor-not-allowed rounded-full transition-colors">

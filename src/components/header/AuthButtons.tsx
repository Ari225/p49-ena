...
export const AuthButtons = ({ isMobile = false, onMobileMenuClose }: AuthButtonsProps) => {
  const { t } = useLanguage();
      ...
      {user ? (
        ...
        <div className="flex space-x-2">
          ...
          <Link to="/login">
            <Button 
              variant="outline" 
              className="border-primary text-primary hover:bg-primary hover:text-white font-medium py-[5px] px-[15px] rounded transition-colors duration-200 text-sm"
            >
              {t('header.login')}
            </Button>
          </Link>

...
const MobileMenu = () => {
  ...
  return <>
      <div className="flex justify-end flex-1">
        <Button variant="ghost" size="sm" onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)} className={`${isTab ? 'p-4' : 'p-2'}`}>
          {isMobileMenuOpen ? <X className={`${isTab ? 'h-7 w-7' : 'h-5 w-5'}`} /> : <Menu className={`${isTab ? 'h-7 w-7' : 'h-5 w-5'}`} />}
        </Button>
      </div>

      {/* Mobile Menu Overlay with slide animation - Full width */}
      <div className={`fixed ${isTab ? 'top-[76px]' : 'top-[70px]'} left-0 right-0 w-screen bg-white shadow-lg border-t border-gray-200 max-h-[calc(100vh-76px)] overflow-y-auto z-40 transform transition-all duration-300 ease-in-out ${isMobileMenuOpen ? 'translate-x-0 opacity-100' : '-translate-x-full opacity-0 pointer-events-none'}`}>
        <nav className="px-[25px] py-[15px]">
          {menuItems.map(menu => <div key={menu.title}>
              ...
              {openSubmenu === menu.title && <div className="bg-white ">
